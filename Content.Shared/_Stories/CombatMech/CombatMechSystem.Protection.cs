...
#pragma warning restore CS0618
        ...
        out DamageSpecifier remaining)
    {
        ...
        foreach (var (type, amount) in damage.DamageDict)
        {
            ...
            var useDamageContainer = TryDamageContainerSupportsType(vehicle, type, out var supportedByContainer);
            if (useDamageContainer ? supportedByContainer : mech.ForwardedDamageTypes.Contains(type))
    ...
    private bool TryDamageContainerSupportsType(EntityUid vehicle, string type, out bool supported)
    {
        supported = false;
        if (!TryComp(vehicle, out DamageableComponent? damageable) ||
            damageable.DamageContainerID == null ||
            !_proto.TryIndex<DamageContainerPrototype>(damageable.DamageContainerID, out var container))
        {
            return false;
        }
